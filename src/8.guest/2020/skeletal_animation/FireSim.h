...
public:
    ...
    void updateFire(float dt) {
        float swirlTimeSpan = 1.0f, swirlOmegaForce = 20.0f, swirlDamping = 10.0f * dt;
        float swirlProb = swirlProbability * h * h;
        float fireCooling = 1.2f * dt, smokeCooling = 0.3f * dt, lift = 3.0f, acceleration = 6.0f * dt;
        ...
        float minR = 0.85f * obstacleRadius, maxR = obstacleRadius + h;
        for (int i = 0; i < numX; i++) {
            for (int j = 0; j < numY; j++) {
                ...
                if (burningObstacle) {
                    float dx = (i + 0.5f) * h - obstacleX, dy = (j + 0.5f) * h - obstacleY - 3.0f * h;
                    float d = dx * dx + dy * dy;
