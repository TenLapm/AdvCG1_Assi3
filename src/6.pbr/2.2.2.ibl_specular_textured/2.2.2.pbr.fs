...
{        
    ...
    // Rotate normal/reflection vectors to match the rotating skybox
    vec3 N_env = vec3(inverse(envRotation) * vec4(N, 0.0));
    vec3 R_env = vec3(inverse(envRotation) * vec4(R, 0.0));
    ...
    vec3 irradiance = texture(irradianceMap, N_env).rgb;
